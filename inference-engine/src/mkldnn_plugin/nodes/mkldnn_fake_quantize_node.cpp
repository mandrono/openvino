...
MKLDNNFakeQuantizeNode::MKLDNNFakeQuantizeNode(const std::shared_ptr<ngraph::Node>& op, const mkldnn::engine& eng, MKLDNNWeightsSharing::Ptr &cache) :
    ...
    if (isSupportedOperation(op, errorMessage)) {
        ...
        if (levels <= 1)
            THROW_IE_EXCEPTION << errorPrefix << "supports 'levels' attribute greater than or equal to 2";

        if (fq->get_input_size() != 5)
            THROW_IE_EXCEPTION << errorPrefix << "has incorrect number of input edges: " << fq->get_input_size();
        if (fq->get_output_size() != 1)
            THROW_IE_EXCEPTION << errorPrefix << "has incorrect number of output edges: " << fq->get_output_size();
        ...
        if (axisSize != -1 && axisSize != axisRealSize)
            THROW_IE_EXCEPTION << errorPrefix << "has different quantization axis size on 'data' and 'range' inputs";
