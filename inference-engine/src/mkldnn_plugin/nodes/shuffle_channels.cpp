    ...
    explicit ShuffleChannelsImpl(const std::shared_ptr<ngraph::Node>& op) {
        try {
            ...
            if (axis < 0 || axis >= static_cast<int64_t>(dstDims.size()))
                THROW_IE_EXCEPTION << op->get_friendly_name() << " Incorrect input parameters dimensions and axis number!";

            size_t group = scOp->get_group();
            if (group == 0 || dstDims[axis] % group)
                THROW_IE_EXCEPTION << op->get_friendly_name() << " Group parameter must evenly divide the channel dimension!";
            ...
            if (dataLength == 0)
                THROW_IE_EXCEPTION << op->get_friendly_name() << " Incorrect input parameters dimension!";
