...
std::tuple<size_t, size_t, size_t, size_t, size_t> MKLDNNMVNNode::get5dShapes(const SizeVector& dims) {
    ...
    switch (dims.size()) {
        ...
        default : { THROW_IE_EXCEPTION << "MVN layer with name '" << getName() << "' doesn't support planar layout with rank: " << dims.size(); }
    }
    return shapes;
}
...
void MKLDNNMVNNode::execute(mkldnn::stream strm) {
    ...
    if (mayiuse(cpu::x64::sse41)) {
        if (!mvn_mean_kernel || (normalizeVariance_ && !mvn_variance_kernel) || !mvn_kernel) {
            THROW_IE_EXCEPTION << "MVN layer with name '" << getName() << "' doesn't create kernel to execute on sse41 above platform.";
        }
