...
void MKLDNNReduceNode::getSupportedDescriptors() {
    ...
    if (getParentEdges().size() != 2)
        THROW_IE_EXCEPTION << errorPrefix << " gets incorrect number of input edges!";
    if (getChildEdges().empty())
        THROW_IE_EXCEPTION << errorPrefix << " gets incorrect number of output edges!";

    if (getParentEdgeAt(REDUCE_INDEXES)->getDims().ndims() != 1) {
        THROW_IE_EXCEPTION << errorPrefix << " gets incorrect index vector dimension! Index vector should be 1 dimension.";
    }

    if (keep_dims) {
        if (getParentEdgeAt(REDUCE_DATA)->getDims().ndims() != getChildEdgeAt(0)->getDims().ndims())
            THROW_IE_EXCEPTION << errorPrefix << " gets incorrect number of input/output dimensions!";
    } else {
...
void MKLDNNReduceNode::createPrimitive() {
    ...
    if (!srcDataMemPtr || !srcDataMemPtr->GetPrimitivePtr() || !srcIndexesMemPtr || !srcIndexesMemPtr->GetPrimitivePtr())
        THROW_IE_EXCEPTION << errorPrefix << " has not allocate input memory.";
...
inline void MKLDNNReduceNode::init_dst_data(uint8_t *out_ptr, size_t dst_size) {
    switch (algorithm) {
        ...
        case ReduceMin:
            ...
            break;
        default:
            THROW_IE_EXCEPTION << errorPrefix << " gets unsupported reduce mode.";
    }
}
...
inline void MKLDNNReduceNode::calc_process_dst_dims(const int32_t *idx_data) {
    ...
    for (size_t i = 0; i < getParentEdgeAt(REDUCE_INDEXES)->getDims()[0]; i++) {
        ...
        if (static_cast<size_t>(axis) > src_dims.size())
            THROW_IE_EXCEPTION << errorPrefix << " exceeds data tensor dimension on index to reduce";
    ...
    for (size_t i = 0; i < std::min(out_dims.size(), dst_dims.size()); i++) {
        if (out_dims[i] != dst_dims[i])
            THROW_IE_EXCEPTION << errorPrefix << "gets incorrect number of output dimensions!";
    }
}
