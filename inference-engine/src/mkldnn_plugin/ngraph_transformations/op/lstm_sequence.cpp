...
constexpr ngraph::NodeTypeInfo MKLDNNPlugin::LSTMSequenceNode::type_info;

MKLDNNPlugin::LSTMSequenceNode::LSTMSequenceNode(const ngraph::Output<ngraph::Node>& X,
                                                 ...
                                                 float clip,
                                                 int64_t seq_axis)
    : ngraph::op::util::RNNCellBase({X, H_t, C_t, sequence_lengths, W, R, B}, hidden_size, clip, activations, activations_alpha, activations_beta),
          m_direction(direction),
          m_seq_axis(seq_axis) {
    constructor_validate_and_infer_types();
}

std::shared_ptr<ngraph::Node> MKLDNNPlugin::LSTMSequenceNode::clone_with_new_inputs(const ngraph::OutputVector& new_args) const {
    check_new_args_count(this, new_args);
    return std::make_shared<MKLDNNPlugin::LSTMSequenceNode>(new_args.at(0), new_args.at(1), new_args.at(2), new_args.at(3),
             new_args.at(4), new_args.at(5), new_args.at(6), m_hidden_size, m_direction, m_activations, m_activations_alpha, m_activations_beta, m_clip,
             m_seq_axis);
}

void MKLDNNPlugin::LSTMSequenceNode::validate_and_infer_types() {
    set_output_size(3);
    ngraph::Shape output_shape_0;
    ngraph::Shape output_shape_1;
    ngraph::Shape output_shape_2;
    ...
    output_shape_2 = ngraph::Shape{batch_size, num_directions, m_hidden_size};
...
bool MKLDNNPlugin::LSTMSequenceNode::visit_attributes(ngraph::AttributeVisitor &visitor) {
