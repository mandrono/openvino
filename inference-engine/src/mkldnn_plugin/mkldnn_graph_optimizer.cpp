...
//    graph.RemoveDroppedNodes();
    ...
    FuseScaleShiftAndFakeQuantize(graph);
    graph.RemoveDroppedNodes();
...
void MKLDNNGraphOptimizer::FuseMultiplyAndAdd(MKLDNNGraph &graph) {
    ...
    auto isSutableParentNode = [&](MKLDNNNodePtr node) {
        if (node->getAlgorithm() != EltwiseMultiply || !node->getFusedWith().empty() ||
            node->getParentEdges().size() != 2 || node->getChildEdges().size() != 1 || node->isConstant())
            return false;
...
//            auto* fakeQuantizeNode = dynamic_cast<MKLDNNFakeQuantizeNode*>(childNode.get());
//            if (fakeQuantizeNode == nullptr)
//                THROW_IE_EXCEPTION << "Cannot get Fakequantize layer " << childNode->getName();
//
//            if (parentNode->getParentEdgesAtPort(0)[0]->getDims().ndims() != 3) {
//                return !fakeQuantizeNode->isBinarization();
//            } else {
//                return (fakeQuantizeNode->isInputLowBroadcast() && fakeQuantizeNode->isInputHighBroadcast() &&
//                        fakeQuantizeNode->isOutputLowBroadcast() && fakeQuantizeNode->isOutputHighBroadcast() &&
//                        !fakeQuantizeNode->isBinarization());
//            }
...
void MKLDNNGraphOptimizer::FuseConvolutionAndSimpleOperation(MKLDNNGraph &graph) {
    ...
    auto isSutableParentNode = [](MKLDNNNodePtr node) {
        return !node->isConstant() && node->getType() == Convolution &&
               node->getChildEdges().size() == 1;
    };
...
void MKLDNNGraphOptimizer::FuseBinaryConvolutionAndFakeQuantize(MKLDNNGraph &graph) {
    ...
    auto isSutableParentNode = [](MKLDNNNodePtr node) {
        ...
        return !node->isConstant() && isSutableBinConv && node->getChildEdges().size() == 1;
    };
...
void MKLDNNGraphOptimizer::FuseConvolutionSumAndConvolutionSumActivation(MKLDNNGraph &graph) {
    ...
    for (auto &graphNode : graphNodes) {
        ...
        bool isSutableParent1 = !parent1->isConstant() && (parent1->getType() == Convolution || parent1->getType() == BinaryConvolution);
        bool isSutableParent2 = !parent2->isConstant() && (parent2->getType() == Convolution || parent2->getType() == BinaryConvolution);
...
void MKLDNNGraphOptimizer::FuseMVNAndSimpleOperation(MKLDNNGraph &graph) {
    ...
    auto isSutableParentNode = [](MKLDNNNodePtr node) {
        bool isSutableMVN = !node->isConstant() && (node->getType() == MVN) && (node->inDims[0].ndims() == 4 || node->inDims[0].ndims() == 5);
    ...
    auto isSutableChildNode = [](MKLDNNNodePtr node) {
        if (node->getType() == FakeQuantize) {
            auto* fakeQuantizeNode = dynamic_cast<MKLDNNFakeQuantizeNode*>(node.get());
            if (fakeQuantizeNode == nullptr)
                THROW_IE_EXCEPTION << "CPU node with name '" << node->getName() << "' is not a FakeQuantize node.";
            return !fakeQuantizeNode->isBinarization();
        } else if (node->getType() == Eltwise) {
...
void MKLDNNGraphOptimizer::FuseEltwiseAndSimple(MKLDNNGraph &graph) {
    ...
    auto isSutableParentNode = [](MKLDNNNodePtr node) {
        return !node->isConstant() && node->getType() == Eltwise && node->getChildEdges().size() == 1;
    };
...
void MKLDNNGraphOptimizer::FuseClampAndFakeQuantize(MKLDNNGraph &graph) {
    ...
    auto isSutableFakeQuantizeNode = [](MKLDNNNodePtr node) {
        ...
        auto* fakeQuantizeNode = dynamic_cast<MKLDNNFakeQuantizeNode*>(node.get());
        if (fakeQuantizeNode == nullptr)
            THROW_IE_EXCEPTION << "Cannot cast " << node->getName() << " to FakeQuantize node";
    ...
    auto fuseClampAndFakeQuantizeNodes = [](MKLDNNNodePtr parent, MKLDNNNodePtr child) {
        ...
        if (fakeQuantizeNode == nullptr)
            THROW_IE_EXCEPTION << "Cannot cast " << child->getName() << " to FakeQuantize node";
...
void MKLDNNGraphOptimizer::FuseScaleShiftAndFakeQuantize(MKLDNNGraph &graph) {
    auto& graphNodes = graph.GetNodes();
    ...
    auto isSutableFakeQuantizeNode = [](MKLDNNNodePtr node) {
        return node->getType() == FakeQuantize && node->getAlgorithm() != FakeQuantizeBinarization;
    };
...
void MKLDNNGraphOptimizer::MergeTransposeAndReorder(MKLDNNGraph &graph) {
    ...
    auto mergeTransposeAndReorder = [&](std::shared_ptr<MKLDNNNode>& parentNode, std::shared_ptr<MKLDNNNode>& childNode) {
        ...
        if (!edge) {
            THROW_IE_EXCEPTION << "Transpose node '" << parentNode->getName() << "' has invalid edges.";
        }
